  ...
  return (
    <div className="p-4 lg:p-6 space-y-4 animate-fade-in">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h2 className="text-2xl lg:text-3xl font-black text-[#263238] dark:text-[#E8F5E9]">İşlemler</h2>
          <p className="text-[#546E7A] dark:text-[#A5D6A7] text-xs lg:text-sm mt-1">{filtered.length} işlem listeleniyor</p>
        </div>
          ...
          {[['all','Tümü'],['income','↑ Gelir'],['expense','↓ Gider']].map(([val, label]) => (
            <button key={val} id={`filter-${val}`} onClick={() => setFilter(val)}
              className={`px-4 py-2 rounded-lg text-xs font-semibold transition-all ${
                filter === val ? 'text-white shadow-md' : 'text-[#546E7A] dark:text-[#A5D6A7] hover:text-[#2E7D32]'
              }`}
              ...
              <div key={tx.id} id={`tx-${tx.id}`}
                ...
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    <p className="text-[#263238] dark:text-[#E8F5E9] text-sm font-semibold truncate">{tx.description}</p>
                    <div className="flex items-center gap-1.5 mt-0.5 flex-wrap">
                      <span className="text-[#78909C] dark:text-[#81C784] text-xs">{tx.category}</span>
                      {tx.subtype && (
                      ...
                      <span className="text-[#78909C] dark:text-[#81C784] text-xs hidden sm:inline">{fmtDate(tx.date)}</span>
                    </div>
                  </div>
                </div>
                <div className="flex items-center gap-1.5 flex-shrink-0 ml-2">
                  <span className={`font-bold text-sm ${tx.type === 'income' ? 'text-[#2E7D32] dark:text-[#66BB6A]' : 'text-red-500'}`}>
                  ...
                  <button onClick={() => handleDelete(tx.id)} id={`delete-tx-${tx.id}`}
                    className="opacity-0 group-hover:opacity-100 text-[#90A4AE] hover:text-red-500 transition-all p-1.5 rounded-lg hover:bg-red-50 dark:hover:bg-red-900/20">
                    <Trash2 size={14} />
                  </button>
                </div>
              </div>
