...
const GRADE_COLOR = { A:'text-[#4F46E5] dark:text-[#A5B4FC]', B:'text-blue-600 dark:text-blue-400', C:'text-amber-600 dark:text-amber-400', D:'text-orange-600 dark:text-orange-400', F:'text-red-600 dark:text-red-400' }
...
export default function Coach() {
  ...
  return (
    <div className="p-4 lg:p-6 space-y-5 animate-fade-in">
      <div>
        <h2 className="text-2xl lg:text-3xl font-black text-[#111827] dark:text-[#EEF2FF]">Finans Koçu 🧠</h2>
        <p className="text-[#6B7280] dark:text-[#A5B4FC] text-xs lg:text-sm mt-1">Kişisel finansal analiz ve tavsiyeler</p>
      </div>
        ...
        <div className="flex flex-col md:flex-row items-center gap-8">
          <div className="relative flex-shrink-0">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              <span className="text-4xl font-black text-[#111827] dark:text-[#EEF2FF]">{data.score}</span>
              <span className="text-[#9CA3AF] dark:text-[#818CF8] text-xs">/ 100</span>
            </div>
          </div>

          <div className="flex-1 space-y-4 w-full">
            <div className="flex items-center gap-3">
              <span className="text-[#6B7280] dark:text-[#A5B4FC] text-sm">Finansal Notun:</span>
              <span className={`text-4xl lg:text-5xl font-black ${GRADE_COLOR[data.grade] || 'text-[#111827] dark:text-[#EEF2FF]'}`}>{data.grade}</span>
            </div>
            <div className="grid grid-cols-2 gap-x-6 gap-y-3">
              {[
                ['Toplam Gelir',   fmt(data.totalIncome),  'text-[#4F46E5] dark:text-[#A5B4FC]'],
                ...
                ['Tasarruf Oranı', `%${data.savingsRate}`, 'text-[#4F46E5] dark:text-[#A5B4FC]'],
                ['Lüks Harcama',   `%${data.luxuryRate}`,  'text-amber-600 dark:text-amber-400'],
                ['Bütçe Kullanım', `%${data.budgetUsage}`, data.budgetUsage > 90 ? 'text-red-600 dark:text-red-400' : 'text-[#4F46E5] dark:text-[#A5B4FC]'],
              ].map(([label, val, color]) => (
                <div key={label}>
                  <p className="text-[#9CA3AF] dark:text-[#818CF8] text-xs">{label}</p>
      ...
      <div className="space-y-3">
        <h3 className="text-[#111827] dark:text-[#EEF2FF] font-bold text-lg">Koç Tavsiyeleri</h3>
          ...
          return (
            <div key={i} id={`advice-${i}`}
              ...
              <div>
                ...
                <p className="text-[#6B7280] dark:text-[#A5B4FC] text-sm mt-0.5 leading-relaxed">{a.message}</p>
              </div>
            </div>
          )
        })}
