...
export default function Dashboard() {
  ...
  if (loading) return (
    <div className="flex items-center justify-center h-full">
      <div className="text-center">
        <div className="w-12 h-12 border-2 border-[#FF6B6B] border-t-transparent rounded-full animate-spin mx-auto mb-3" />
  ...
  const cards = [
    ...
    { id: 'stat-savings',  title: 'Tasarruf Oranı', val: `%${savings.toFixed(1)}`,
      Icon: PiggyBank,    cls: 'bg-[#FFF0E4] dark:bg-[#1a1a1a] border-[#FFB3B3] dark:border-[#5a1a1a]', icon: 'text-[#FF6B6B] dark:text-[#4D96FF]' },
  ]
  ...
  return (
    <div className="p-4 lg:p-6 space-y-5 animate-fade-in">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h2 className="text-2xl lg:text-3xl font-black text-[#2D2D2D] dark:text-[#FFF0E4]">Dashboard</h2>
          <p className="text-[#6B7280] dark:text-[#FFB3B3] text-xs lg:text-sm mt-1">{new Date().toLocaleDateString('tr-TR', { weekday:'long', year:'numeric', month:'long', day:'numeric' })}</p>
        </div>
        ...
        {cards.map(({ id, title, val, Icon, cls, icon }) => (
          <div key={id} id={id} className={`stat-card ${cls}`}>
            <div className="flex items-center justify-between mb-3">
              <p className="text-[#6B7280] dark:text-[#FF9999] text-xs font-semibold uppercase tracking-wide">{title}</p>
              <Icon size={18} className={`${icon} opacity-80`} />
            </div>
            <p className="text-xl font-bold text-[#2D2D2D] dark:text-[#FFF0E4]">{val}</p>
          </div>
        ))}
      </div>
      ...
      <div className="glass-card p-5 lg:p-6">
        <div className="flex items-center justify-between mb-3">
          <div>
            <h3 className="text-[#2D2D2D] dark:text-[#FFF0E4] font-bold">Aylık Bütçe</h3>
            <p className="text-[#9CA3AF] dark:text-[#FF9999] text-xs mt-0.5">Limit: {fmt(budget?.monthlyLimit || 0)}</p>
          </div>
          <div className="text-right">
            <span className={`text-2xl font-black ${pct >= 90 ? 'text-red-500' : pct >= 75 ? 'text-[#FFD93D]' : 'text-[#FF6B6B] dark:text-[#4D96FF]'}`}>
              %{pct.toFixed(0)}
            </span>
            <p className="text-[#9CA3AF] dark:text-[#FF9999] text-xs">kullanıldı</p>
          </div>
        </div>
        <div className="h-3 bg-[#FFF0E4] dark:bg-[#1a1a1a] rounded-full overflow-hidden border border-[#FFDCC8] dark:border-[#4a1a1a]">
          <div className={`h-full ${pctBg} rounded-full transition-all duration-1000`}
               style={{ width: `${Math.min(100, pct)}%` }} />
        </div>
        <div className="flex justify-between text-xs text-[#9CA3AF] dark:text-[#FF9999] mt-2">
          <span>Harcanan: <span className="text-[#2D2D2D] dark:text-[#FFF0E4] font-medium">{fmt(budget?.totalExpenses || 0)}</span></span>
          <span>Kalan: <span className={budget?.remaining >= 0 ? 'text-[#FF6B6B] dark:text-[#4D96FF] font-medium' : 'text-red-500 font-medium'}>{fmt(budget?.remaining || 0)}</span></span>
        </div>
      </div>
      ...
      <div className="glass-card p-5 lg:p-6">
        ...
        <div className="space-y-1">
          {txs.length === 0 ? (
            <p className="text-[#B0B8C4] dark:text-[#5a1a1a] text-center py-6 text-sm">Henüz işlem yok. Yeni bir işlem ekleyin!</p>
          ) : txs.map(tx => (
            <div key={tx.id} className="flex items-center justify-between py-2.5 border-b border-[#FFF0E4] dark:border-[#1a1a1a] last:border-0 hover:bg-[#FFFAF5] dark:hover:bg-[#1a1a1a] rounded-lg px-2 -mx-2 transition-colors">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-[#2D2D2D] dark:text-[#FFF0E4] text-sm font-medium leading-tight">{tx.description}</p>
                  <p className="text-[#9CA3AF] dark:text-[#FF9999] text-xs mt-0.5">{tx.category} • {fmtDate(tx.date)}</p>
                </div>
              </div>
              <span className={`font-bold text-sm flex-shrink-0 ${tx.type === 'income' ? 'text-[#FF6B6B] dark:text-[#4D96FF]' : 'text-red-500'}`}>
