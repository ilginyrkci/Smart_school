...
export default function AddTransactionModal({ onClose, onSuccess, editTx = null }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(isEdit ? 'Güncelleme sırasında hata oluştu.' : 'İşlem eklenirken hata oluştu.')
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="fixed inset-0 bg-[#263238]/40 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-fade-in">
      <div className="bg-white border border-[#C8E6C9] rounded-2xl w-full max-w-md shadow-2xl animate-slide-up">
        ...
        <div className="flex items-center justify-between p-6 border-b border-[#E8F5E9]">
          ...
          <button onClick={onClose} id="modal-close"
            className="text-[#78909C] hover:text-[#263238] p-1.5 rounded-lg hover:bg-[#E8F5E9] transition-all">
            <X size={18} />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          {/* Type */}
          <div className="flex gap-2 bg-[#F1F8E9] rounded-xl p-1 border border-[#C8E6C9]">
            <button type="button" id="type-income" onClick={() => handleTypeChange('income')}
              className={`flex-1 py-2.5 rounded-lg text-sm font-semibold flex items-center justify-center gap-2 transition-all ${
                form.type === 'income' ? 'bg-[#2E7D32] text-white shadow-md' : 'text-[#546E7A] hover:text-[#2E7D32]'
              }`}>
              <ArrowUpCircle size={15} /> Gelir
            </button>
            <button type="button" id="type-expense" onClick={() => handleTypeChange('expense')}
              className={`flex-1 py-2.5 rounded-lg text-sm font-semibold flex items-center justify-center gap-2 transition-all ${
                form.type === 'expense' ? 'bg-red-500 text-white shadow-md' : 'text-[#546E7A] hover:text-red-500'
              }`}>
          ...
          {/* Category */}
          <div>
            <label className="text-[#546E7A] text-xs font-semibold uppercase tracking-wider mb-1.5 block">Kategori</label>
          ...
          {form.type === 'expense' && (
            <div>
              <label className="text-[#546E7A] text-xs font-semibold uppercase tracking-wider mb-1.5 block">Harcama Tipi</label>
              <div className="flex gap-2">
                {[
                  { val: 'necessary', label: '🏠 Zorunlu', active: 'bg-blue-50 border-blue-300 text-blue-700' },
                  { val: 'luxury',    label: '✨ Lüks',    active: 'bg-amber-50 border-amber-300 text-amber-700' },
                ].map(({ val, label, active }) => (
                  <button key={val} type="button" id={`subtype-${val}`}
                    onClick={() => set('subtype', val)}
                    className={`flex-1 py-2 rounded-xl text-xs font-semibold border transition-all ${
                      form.subtype === val ? active : 'border-[#C8E6C9] text-[#78909C] hover:border-[#A5D6A7] hover:text-[#546E7A]'
                    }`}>
                    {label}
                  </button>
                ))}
          ...
          {/* Amount */}
          <div>
            <label className="text-[#546E7A] text-xs font-semibold uppercase tracking-wider mb-1.5 block">Miktar (₺)</label>
          ...
          {/* Description */}
          <div>
            <label className="text-[#546E7A] text-xs font-semibold uppercase tracking-wider mb-1.5 block">Açıklama</label>
          ...
          {/* Date */}
          <div>
            <label className="text-[#546E7A] text-xs font-semibold uppercase tracking-wider mb-1.5 block">Tarih</label>
