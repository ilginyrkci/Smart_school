    ...
    <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-50 border-t border-[#FFDCC8] dark:border-[#4a1a1a] bg-white dark:bg-[#0f0f0f]">
      <div className="flex items-center justify-around h-16 px-1">
          ...
          <NavLink key={to} to={to}
            className={({ isActive }) =>
              `flex flex-col items-center gap-0.5 px-2 py-1 rounded-xl transition-all min-w-0 flex-1 ${
                isActive
                  ? 'text-[#FF6B6B] dark:text-[#FFB3B3]'
                  : 'text-[#9CA3AF] dark:text-[#FF9999] hover:text-[#FF6B6B]'
              }`
            }>
            {({ isActive }) => (
              <>
                <div className={`p-1.5 rounded-xl transition-all ${isActive ? 'bg-[#FFF0E4] dark:bg-[#1a0f0f]' : ''}`}>
                  <Icon size={20} />
                </div>
        ...
        <button onClick={toggle}
          className="flex flex-col items-center gap-0.5 px-2 py-1 min-w-0 flex-1 text-[#9CA3AF] dark:text-[#FFB3B3]">
