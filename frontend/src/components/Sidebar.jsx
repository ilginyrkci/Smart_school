    ...
    <aside className="hidden lg:flex w-64 flex-shrink-0 flex-col h-screen border-r border-[#FFDCC8] dark:border-[#4a1a1a] bg-white dark:bg-[#0f0f0f]">
      ...
      <div className="p-6 border-b border-[#FFDCC8] dark:border-[#4a1a1a]">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs font-medium text-[#FF6B6B]">Finans Koçu</p>
          </div>
        </div>
      </div>
            ...
            className={({ isActive }) =>
              `flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all duration-200 group ${
                isActive
                  ...
                  : 'text-[#6B7280] dark:text-[#FFB3B3] hover:text-[#FF6B6B] hover:bg-[#FFF0E4] dark:hover:bg-[#1a0f0f]'
              }`
            }
            style={({ isActive }) => isActive ? { background: 'linear-gradient(135deg, #FF6B6B, #4D96FF)' } : {}}>
            {({ isActive }) => (
              <>
                <Icon size={18} className={isActive ? 'text-white' : 'text-[#9CA3AF] dark:text-[#FF9999] group-hover:text-[#FF6B6B]'} />
                {label}
      ...
      <div className="px-3 pb-2">
        <button onClick={toggle} id="btn-theme-toggle"
          className="w-full flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm font-medium transition-all text-[#6B7280] dark:text-[#FFB3B3] hover:bg-[#FFF0E4] dark:hover:bg-[#1a1a1a]">
          {isDark
        ...
        <NavLink to="/app/profile" id="nav-profile"
          className={({ isActive }) =>
            `flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all group cursor-pointer ${
              isActive ? 'bg-[#FFF0E4] dark:bg-[#1a0f0f]' : 'hover:bg-[#FFF0E4] dark:hover:bg-[#1a0f0f]'
            }`
          }>
          ...
          <Settings size={14} className="text-[#B0B8C4] dark:text-[#5a1a1a] group-hover:text-[#FF6B6B] transition-colors flex-shrink-0" />
        </NavLink>
